...
class TravelStudioService:
    ...
    def create_multi_room_booking(
        ...
        **kwargs
        ...
        Args:
            ...
            check_in_date: Check-in date (YYYY-MM-DD)
            check_out_date: Check-out date (YYYY-MM-DD)
            rooms: List of room objects with category/id
        ...
        Example:
            rooms = [
                {"category": "Deluxe", "room_id": "abc123"},
                {"category": "Luxury Cottage", "room_id": "def456"}
            ]
        """
        data = {
            "guest_name": guest_name,
        ...
        if result and result.get("success"):
            logger.info(f"Multi-room booking created successfully with {len(rooms)} rooms")
            return result.get("data")
        return None
    
    def extend_room_stay(
        self,
        booking_id: str,
        ...
        **kwargs
        ...
        """
        Extend the stay for a specific room in a booking
        
        Args:
            booking_id: Booking ID
            room_id_or_number: Room ID or room number
            new_check_out_date: New check-out date (YYYY-MM-DD)
            **kwargs: Additional parameters (e.g., reason, notes)
            
        Returns:
            Updated booking details or None on error
        """
        data = {
        ...
        result = self._make_request(
            "POST",
            f"/api/hocc/bookings/{booking_id}/rooms/{room_id_or_number}/extend",
            data=data
        )
        
        if result and result.get("success"):
            logger.info(f"Room {room_id_or_number} stay extended to {new_check_out_date}")
            return result.get("data")
        return None
    ...
    def upgrade_room(
        ...
        **kwargs
        ...
        Args:
            booking_id: Booking ID
            room_id_or_number: Current room ID or room number
            new_room_category: Target room category (e.g., "Luxury Cottage")
            new_room_id: Specific new room ID (optional)
            **kwargs: Additional parameters (e.g., price_difference)
            
        Returns:
            Updated booking details or None on error
        """
        data = {
            "new_room_category": new_room_category,
            **kwargs
        }
        
        if new_room_id:
            data["new_room_id"] = new_room_id
        
        result = self._make_request(
            "POST",
            f"/api/hocc/bookings/{booking_id}/rooms/{room_id_or_number}/upgrade",
            data=data
        )
        
        if result and result.get("success"):
            logger.info(f"Room {room_id_or_number} upgraded to {new_room_category}")
            return result.get("data")
        return None
    ...
    def add_room_to_booking(
        ...
        **kwargs
        ...
        Args:
            booking_id: Booking ID
            room_category: Room category to add
            room_id: Specific room ID (optional, will auto-assign if not provided)
            check_in_date: Check-in date (optional, uses booking dates if not provided)
            check_out_date: Check-out date (optional, uses booking dates if not provided)
        ...
        result = self._make_request(
            "POST",
            f"/api/hocc/bookings/{booking_id}/rooms",
            data=data
        )
        
        if result and result.get("success"):
            logger.info(f"Room added to booking {booking_id}")
            return result.get("data")
        return None
    
    def update_room_in_booking(
        self,
        booking_id: str,
        room_id_or_number: str,
        **update_fields
        ...
        Args:
            booking_id: Booking ID
            room_id_or_number: Room ID or room number
            **update_fields: Fields to update (e.g., check_in_date, check_out_date, status)
            
        Returns:
            Updated room details or None on error
        """
        result = self._make_request(
            "PUT",
            f"/api/hocc/bookings/{booking_id}/rooms/{room_id_or_number}",
            data=update_fields
        )
        
        if result and result.get("success"):
            logger.info(f"Room {room_id_or_number} in booking {booking_id} updated")
            return result.get("data")
        return None
    
    def remove_room_from_booking(
        self,
        booking_id: str,
        room_id_or_number: str,
        ...
        Args:
            booking_id: Booking ID
            room_id_or_number: Room ID or room number to remove
        ...
        if result and result.get("success"):
            logger.info(f"Room {room_id_or_number} removed from booking {booking_id}")
            return True
        return False
